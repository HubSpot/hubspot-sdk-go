...
func TestSequenceNewEnrollmentWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Automation.Sequences.NewEnrollment(context.TODO(), automation.SequenceNewEnrollmentParams{
		UserID: "userId",
		PublicSequenceEnrollmentRequest: automation.PublicSequenceEnrollmentRequestParam{
			ContactID:          "contactId",
			SenderEmail:        "senderEmail",
			SequenceID:         "sequenceId",
			SenderAliasAddress: hubspotsdk.String("senderAliasAddress"),
		},
	})
	if err != nil {
		var apierr *hubspotsdk.Error
...
func TestSequenceGetEnrollmentByContactID(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Automation.Sequences.GetEnrollmentByContactID(context.TODO(), "contactId")
	if err != nil {
		var apierr *hubspotsdk.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
