...
func TestDealSplitBatchRead(t *testing.T) {
	...
	_, err := client.Crm.DealSplits.Batch.Read(context.TODO(), crm.DealSplitBatchReadParams{
...
func TestDealSplitBatchUpsert(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Crm.DealSplits.Batch.Upsert(context.TODO(), crm.DealSplitBatchUpsertParams{
