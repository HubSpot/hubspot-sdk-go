...
func TestImportNewWithOptionalParams(t *testing.T) {
	...
	_, err := client.Crm.Imports.New(context.TODO(), crm.ImportNewParams{
		Files:         io.Reader(bytes.NewBuffer([]byte("Example data"))),
...
func TestImportListWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
...
func TestImportListErrorsWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Crm.Imports.ListErrors(
		...
		crm.ImportListErrorsParams{
			After:               hubspotsdk.String("after"),
			IncludeErrorMessage: hubspotsdk.Bool(true),
			IncludeRowData:      hubspotsdk.Bool(true),
			Limit:               hubspotsdk.Int(0),
		},
	)
	if err != nil {
