...
func TestObjectPartnerClientBatchUpdate(t *testing.T) {
	...
	_, err := client.Crm.Objects.PartnerClients.Batch.Update(context.TODO(), crm.ObjectPartnerClientBatchUpdateParams{
...
func TestObjectPartnerClientBatchGetWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Crm.Objects.PartnerClients.Batch.Get(context.TODO(), crm.ObjectPartnerClientBatchGetParams{
