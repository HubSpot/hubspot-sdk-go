...
// provided.
func (r *ObjectNoteService) New(ctx context.Context, body ObjectNoteNewParams, opts ...option.RequestOption) (res *CreatedResponseSimplePublicObject, err error) {
...
func (r *ObjectNoteService) Update(ctx context.Context, noteID string, params ObjectNoteUpdateParams, opts ...option.RequestOption) (res *SimplePublicObject, err error) {
	...
	path := fmt.Sprintf("crm/v3/objects/notes/%s", noteID)
...
type ObjectNoteListParams struct {
	...
	// the maximum number of notes that can be read by a single request.
