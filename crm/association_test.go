...
func TestAssociationListWithOptionalParams(t *testing.T) {
	...
	_, err := client.Crm.Associations.List(
		context.TODO(),
		"toObjectType",
...
func TestAssociationRequestHighUsageReport(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Crm.Associations.RequestHighUsageReport(context.TODO(), 0)
	if err != nil {
		var apierr *hubspotsdk.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
...
func TestAssociationSearchWithOptionalParams(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Crm.Associations.Search(
		context.TODO(),
		"objectType",
