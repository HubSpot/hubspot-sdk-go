...
// provided.
func (r *ObjectFeeService) New(ctx context.Context, body ObjectFeeNewParams, opts ...option.RequestOption) (res *CreatedResponseSimplePublicObject, err error) {
...
func (r *ObjectFeeService) Update(ctx context.Context, feeID string, params ObjectFeeUpdateParams, opts ...option.RequestOption) (res *SimplePublicObject, err error) {
	...
	path := fmt.Sprintf("crm/v3/objects/fees/%s", feeID)
...
type ObjectFeeListParams struct {
	...
	// the maximum number of fees that can be read by a single request.
