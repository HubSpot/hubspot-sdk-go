...
func TestMarketingEventSettingNewOrUpdate(t *testing.T) {
	...
	_, err := client.Marketing.MarketingEvents.Settings.NewOrUpdate(
		context.TODO(),
		0,
...
func TestMarketingEventSettingGet(t *testing.T) {
	t.Skip("Mock server tests are disabled")
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	...
	_, err := client.Marketing.MarketingEvents.Settings.Get(context.TODO(), 0)
	if err != nil {
		var apierr *hubspotsdk.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
