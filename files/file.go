...
// Returns the unmodified JSON received from the API
func (r FileStat) RawJSON() string { return r.JSON.raw }
func (r *FileStat) UnmarshalJSON(data []byte) error {
	return apijson.UnmarshalRoot(data, r)
}

// Object for updating files.
type FileUpdateInputParam struct {
	ClearExpires param.Opt[bool]      `json:"clearExpires,omitzero"`
...
// The properties Access, URL are required.
type ImportFromURLInputParam struct {
	...
	Overwrite param.Opt[bool] `json:"overwrite,omitzero"`
	...
	DuplicateValidationScope ImportFromURLInputDuplicateValidationScope `json:"duplicateValidationScope,omitzero"`
	// NONE: Do not run any duplicate validation. REJECT: Reject the upload if a
	// duplicate is found. RETURN_EXISTING: If a duplicate file is found, do not upload
	// a new file and return the found duplicate instead.
	//
	// Any of "NONE", "REJECT", "RETURN_EXISTING".
	DuplicateValidationStrategy ImportFromURLInputDuplicateValidationStrategy `json:"duplicateValidationStrategy,omitzero"`
	paramObj
}
